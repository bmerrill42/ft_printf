...
enum						e_flag {
	...
	Z_FLAG = 0x0400,
	WIDTH_FLAG = 0x0800,
	PRECISION_FLAG = 0x1000,
	C_NULL_FLAG = 0x2000
};

typedef unsigned short		t_flags;
typedef __builtin_va_list	t_va_list;
...
char						*padding(int size, char padding_char,
										int arg_length);
char						*do_width(intmax_t arg, char *ret,
										t_optional *options);
char						*do_width_u(uintmax_t arg, char *ret,
										t_optional *options);
char						*apply_flags_u(t_optional *options, char *ret,
											uintmax_t arg);
...
char						*apply_flags(t_optional *options, char *ret,
											intmax_t arg);
